<template>
  ...
  <!-- Encabezado -->
  <div class="px-3 py-2 sm:px-4 sm:py-3 bg-indigo-50 dark:bg-indigo-900/20 border-b border-gray-100 dark:border-gray-800">
    <h3 class="text-base font-medium text-gray-900 dark:text-white flex items-center">
      <span class="inline-flex items-center justify-center w-6 h-6 rounded-lg bg-indigo-100 dark:bg-indigo-800 text-indigo-600 dark:text-indigo-300 mr-2">
        <!-- ícono -->
        <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 2a1 1 0 011 1v1.323l3.954 1.582 1.599-.8a1 1 0 01.894 1.79l-1.233.616 1.738 5.42a1 1 0 01-.285 1.05A3.989 3.989 0 0115 15a3.989 3.989 0 01-2.667-1.019 1 1 0 01-.285-1.05l1.715-5.349L11 6.477V16h2a1 1 0 110 2H7a1 1 0 110-2h2V6.477L6.237 7.582l1.715 5.349a1 1 0 01-.285 1.05A3.989 3.989 0 015 15a3.989 3.989 0 01-2.667-1.019 1 1 0 01-.285-1.05l1.738-5.42-1.233-.617a1 1 0 01.894-1.788l1.599.799L9 4.323V3a1 1 0 011-1z" clip-rule="evenodd"/></svg>
      </span>
    ...
    <div class="space-y-1 sm:space-y-2 mb-3">
      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3 transition-all duration-300">
        <div class="flex items-center justify-between mb-1 sm:mb-2">
          <label class="text-xs font-medium text-gray-900 dark:text-white">Buscar por Nombre</label>
          <button v-if="filters.name" @click="filters.name='';debounceSearch()" type="button"
                  class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition duration-200">
          ...
          <input v-model="filters.name" @input="debounceSearch" type="text"
                 class="pl-10 w-full text-xs sm:text-sm rounded-md border-0 py-2 text-gray-900 dark:text-white ring-1 ring-inset ring-gray-300 dark:ring-gray-700 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 bg-transparent"
    ...
    <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3 transition-all duration-300">
      <h3 class="text-xs font-medium text-gray-900 dark:text-white mb-2">
        ...
        <span class="text-gray-500 dark:text-gray-400">({{ drugs.total }})</span>
        <span class="text-red-500">*</span>
      </h3>

      <div class="max-h-60 sm:max-h-[180px] overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 mb-2">
        <div v-for="drug in drugs.data"
             :key="drug.id"
             @click="selectDrug(drug)"
             class="group relative py-1.5 px-2.5 sm:py-2 sm:px-3 cursor-pointer border-b border-gray-100 dark:border-gray-800 last:border-0 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition duration-200"
             :class="selectedDrugLocal===drug.id ? 'bg-indigo-100 dark:bg-indigo-900/30' : ''">
          <div class="flex justify-between items-center">
            <div>
              <span class="font-medium text-gray-900 dark:text-white text-sm">{{ drug.name }}</span>
              <div v-if="selectedDrugId===drug.id" class="text-xs text-green-500 dark:text-green-400 flex items-center">
            ...
            <div class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
              {{ formatDate(drug.created_at) }}
                        ...
                        <!-- Tooltip -->
          <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-200 absolute -top-1 left-1/2 -translate-x-1/2 -translate-y-full z-50 pointer-events-none">
            <div class="bg-gray-900 dark:bg-gray-700 text-white text-[10px] sm:text-xs rounded py-1 px-2 max-w-xs text-center shadow-lg">
              <p class="font-medium mb-1">{{ drug.name }}</p>
              <p class="text-gray-200 dark:text-gray-300">{{ drug.description || 'Sin descripción disponible.' }}</p>
            </div>
            <div class="w-3 h-3 bg-gray-900 dark:bg-gray-700 transform rotate-45 absolute left-1/2 -translate-x-1/2 -bottom-1"></div>
               ...
               <div class="flex flex-wrap gap-2 sm:flex-nowrap sm:space-x-2 items-center justify-between">
        ...
        <div class="flex gap-2 sm:gap-0 sm:space-x-2">
          <button @click="prevPage" :disabled="!drugs.prev_page_url" type="button"
                  class="px-2 py-1 text-[10px] sm:text-xs font-medium rounded-md transition duration-200"
          ...
          <button @click="nextPage" :disabled="!drugs.next_page_url" type="button"
                  class="px-2 py-1 text-[10px] sm:text-xs font-medium rounded-md transition duration-200"
          ...
          <button @click="openCreateModal" type="button"
                  class="px-2 py-1 text-[10px] sm:text-xs font-medium rounded-md bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-200 dark:hover:bg-indigo-800/50 transition duration-200">
            <span class="flex items-center">
              <svg class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <transition name="fade" mode="out-in">
      <div v-if="selectedDrugLocal"
        ...
        <div class="flex items-center space-x-2">
          ...
          <div>
            <p class="text-[10px] sm:text-xs font-medium text-gray-900 dark:text-white">Medicamento seleccionado</p>
            <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
              {{ drugs.data.find(d => d.id === selectedDrugLocal)?.name }}
            </p>
          </div>
        </div>
      </div>
