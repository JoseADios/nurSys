...
<AppLayout>
    ...
    <div class="flex items-center justify-end py-2">
        <Link :href="route('medicalOrders.index')" v-if="admission_id" class="mr-6 inline-flex items-center text-sm font-medium text-gray-700 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-500 self-end">
        Remover filtro de
        ...
        <div class="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-2 md:content-end md:justify-end">
            ...
            <AccessGate :permission="['medicalOrders.create']">
                <div class="w-full sm:w-auto">
                    <Link v-if="!form.admission_id" :href="route('medicalOrders.create')" class="flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-3 rounded-lg whitespace-nowrap text-sm">
                    <PlusIcon class="size-5" />
                    <span class="">Nueva Órden</span>
                    </Link>
                    <Link v-else :href="route('medicalOrders.create', { admission_id: form.admission_id })" class="flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-3 rounded-lg whitespace-nowrap text-sm">
                    <PlusIcon class="size-5" />
        ...
        <div class="min-w-full overflow-x-auto">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-6 py-3 cursor-pointer whitespace-nowrap hidden sm:table-cell" @click="sort('medical_orders.id')">
