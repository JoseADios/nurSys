...
class MedicationRecordController extends Controller implements HasMiddleware
{
    ...
    {
        return [new Middleware('permission:medicationRecord.view', only: ['index', 'show']), new Middleware('permission:medicationRecord.create', only: ['store']), new Middleware('permission:medicationRecord.update', only: ['update']), new Middleware('permission:medicationRecord.delete', only: ['destroy'])];
    }
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($sortField) {
            ...
            $query->latest('medication_records.updated_at')->latest('medication_records.created_at');
        }
        if ($days) {
            $query->where('medication_records.created_at', '>=', now()->subDays($days)->startOfDay());
        }
   ...
   public function store(Request $request)
{
    ...
    public function destroy(MedicationRecord $medicationRecord)
    {
        ...
        $hasNotifications = MedicationNotification::whereIn('medication_record_detail_id', function ($query) use ($medicationRecord) {
            $query->select('id')->from('medication_record_details')->where('medication_record_id', $medicationRecord->id);
        })
            ->where('applied', 1)
            ->get();
        ...
        MedicationNotification::whereIn('medication_record_detail_id', $details->pluck('id'))->update(['active' => 0]);
